...
const Index = () => {
    ...
    const handleShopifyConnection = async () => {
      ...
      if (shopDomain && isShopifyRequest) {
        ...
        try {
          ...
          const { data: existingStore } = await shopifyStores().select('*').eq('shop', shopDomain).maybeSingle();
          if (existingStore && existingStore.access_token) {
            ...
            toast({ title: 'تم الاتصال بنجاح', description: `تم الاتصال بمتجر ${shopDomain}` });
          ...
          toast({ title: 'خطأ في التحقق من المتجر', description: 'حدث خطأ أثناء التحقق من حالة المتجر', variant: 'destructive' });
        } finally {
  ...
  const handleShopifyConnected = (shop: string) => {
    ...
    toast({ title: 'تم الاتصال بنجاح', description: `تم الاتصال بمتجر ${shop}` });
    navigate('/dashboard');
  };
  ...
  return (
    <div dir={language === 'ar' ? 'rtl' : 'ltr'} className="min-h-screen">
      ...
      {isProcessingShopify && (
        <div className="fixed top-0 left-0 right-0 bg-blue-500 text-white text-center py-2 z-50">جاري التحقق من اتصال المتجر...</div>
      )}
          ...
          <div className="max-w-5xl mx-auto bg-codform-light-purple rounded-lg p-8">
            <div className="flex flex-col lg:flex-row-reverse items-center">
              <div className="lg:w-1/2 mb-6 lg:mb-0">
                ...
                <p className="text-gray-700 mb-6 text-right">صمم نموذج الدفع عند الاستلام الخاص بك باستخدام منشئ النماذج السهل والمرن</p>
