...
const ShopifyReconnectButton: React.FC<ShopifyReconnectButtonProps> = ({
  ...
  const handleReconnect = async () => {
    try {
      ...
      const { shopifyConnectionManager } = await import('@/lib/shopify/connection-manager');
      ...
      const shopifyAuthUrl = shopDomain 
        ? `https://7e4608874bbcc38afa1953948da28407.shopifypreview.com/admin/oauth/authorize?client_id=7e4608874bbcc38afa1953948da28407&scope=read_products,read_orders,write_products,write_orders&redirect_uri=${encodeURIComponent(baseUrl + '/api/shopify-callback')}&state=${Date.now()}&shop=${shopDomain}`
        : '/shopify-connect';
  ...
  return (
    <div className="space-y-4">
      <Alert>
        ...
        <AlertDescription>
          {shopDomain 
            ? `المتجر ${shopDomain} غير مربوط بقاعدة البيانات. يجب إعادة ربطه.`
            : 'لا يوجد متجر مربوط. يجب ربط متجر Shopify أولاً.'
          }
        </AlertDescription>
      </Alert>
