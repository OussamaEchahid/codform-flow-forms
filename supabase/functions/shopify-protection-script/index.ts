...
(function() {
  ...
  function blockAccess(blockInfo) {
    ...
    const locationInfo = blockInfo.visitor_country ? 
      '<div class="location-info"><div class="location-text">موقعك الجغرافي: ' + blockInfo.visitor_country + '</div></div>' : '';
    const redirectButton = (blockInfo.redirect_url && blockInfo.redirect_url !== '/blocked') ? 
      '<button class="btn btn-secondary" onclick="window.location.href=\'' + blockInfo.redirect_url + '\'">🔗 انتقال إلى صفحة أخرى</button>' : '';
    
    // مسح كامل الصفحة وكتابة صفحة جديدة من الصفر
    document.open();
