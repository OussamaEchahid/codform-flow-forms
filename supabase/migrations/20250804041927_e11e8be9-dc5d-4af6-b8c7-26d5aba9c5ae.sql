...
DO $$ BEGIN
  IF NOT EXISTS (
    ...
    WHERE tablename = 'custom_currency_symbols' 
...
-- حذف السياسة إذا كانت موجودة وإعادة إنشائها
DROP POLICY IF EXISTS "Users can manage their own currency symbols" ON public.custom_currency_symbols;
...
-- حذف الفهرس إذا كان موجوداً وإعادة إنشائه
...
DROP POLICY IF EXISTS "Users can manage currency display settings" ON public.currency_display_settings;
