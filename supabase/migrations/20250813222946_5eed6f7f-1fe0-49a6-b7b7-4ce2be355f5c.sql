-- حذف الدالة القديمة وإنشاء دالة جديدة واحدة فقط
DROP FUNCTION IF EXISTS public.create_form_for_shop(text, text, text, jsonb, jsonb, boolean);
DROP FUNCTION IF EXISTS public.create_form_for_shop(text, text, text, jsonb, jsonb, boolean, text, text, text);

-- إنشاء دالة واحدة جديدة مع جميع المعاملات
